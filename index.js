...
const HTML_LINES = [
  ...
  "  // Cập nhật dropdown chọn code cho Violentmonkey Loader (nếu đang ở trang codevault)",
  ...
  "}",
  "",
  "/* Sinh nội dung userscript Violentmonkey */",
  "function vmBuildScript({ scriptName, matchUrl, snippetId, snippetName, loadingSec, antiBug, antiDebug, antiEdit, antiConsole }){",
  "  const apiUrl = (window.API_BASE || window.location.origin) + '/api/admin/code-snippets/' + snippetId;",
  "  const loadMs = (parseInt(loadingSec) || 4) * 1000;",
  "",
  "  /* ---- Khối anti-debug: phát hiện DevTools mở → thoát / xoá dữ liệu nhạy cảm ---- */",
  "  const antiDebugBlock = antiDebug ? `",
  "  // Anti-debug: phát hiện DevTools mở bằng kỹ thuật timing",
  "  (function _antiDebug(){",
  "    let _devOpen = false;",
  "    const _check = ()=>{",
  ...
  "  })();` : '';",
  "",
  "  /* ---- Khối khoá console: ghi đè các hàm log/warn/error/dir của DevTools ---- */",
  "  const antiConsoleBlock = antiConsole ? `",
  "  // Khoá DevTools Console",
  ...
  "  })();` : '';",
  "",
  "  /* ---- Khối anti-edit: đóng băng Object.prototype và các object toàn cục nhạy cảm ---- */",
  "  const antiEditBlock = antiEdit ? `",
  "  // Anti-chỉnh sửa runtime: đóng băng các đối tượng toàn cục quan trọng",
  "  (function _antiEdit(){",
  ...
  "  })();` : '';",
  "",
  "  /* ---- Khối anti-bug: wrapper try/catch toàn cục bắt lỗi không mong muốn ---- */",
  "  const antiBugWrap = (inner) => antiBug",
  "    ? `(function _safeguard(){\n  try{\n${inner}\n  }catch(_e){\n    // Lỗi bị bắt bởi anti-bug wrapper — script vẫn tiếp tục\n  }\n})();`",
  "    : inner;",
  "",
  "  /* ---- Giao diện loading overlay ---- */",
  "  const loaderUI = `",
  "  // ---- Loading overlay ----",
  "  const _overlay = document.createElement('div');",
  "  _overlay.id = '__kv_loader__';",
  "  _overlay.style.cssText = [",
  "    'position:fixed','top:0','left:0','width:100%','height:100%',",
  "    'background:rgba(10,10,14,0.93)','z-index:2147483647',",
  "    'display:flex','flex-direction:column',",
  "    'align-items:center','justify-content:center',",
  "    'font-family:system-ui,sans-serif','color:#f1f1f1',",
  "    'user-select:none','pointer-events:all'",
  "  ].join(';');",
  "  _overlay.innerHTML = \\`",
  "    <div style='text-align:center'>",
  "      <div style='width:52px;height:52px;margin:0 auto 18px;border:4px solid #333;border-top-color:#c99a2e;border-radius:50%;animation:__kvSpin 0.75s linear infinite'></div>",
  "      <div style='font-size:15px;font-weight:600;letter-spacing:.3px;margin-bottom:6px'>Đang tải dữ liệu server...</div>",
  "      <div id='__kv_pct' style='font-size:12px;color:#aaa;margin-bottom:18px'>0%</div>",
  "      <div style='width:220px;height:4px;background:#222;border-radius:99px;overflow:hidden'>",
  "        <div id='__kv_bar' style='height:100%;width:0%;background:linear-gradient(90deg,#ad7f1e,#c99a2e);border-radius:99px;transition:width .18s ease'></div>",
  "      </div>",
  "      <div style='margin-top:14px;font-size:11px;color:#555'>KeyVault · Secure Loader</div>",
  "    </div>",
  "  \\`;",
  "  const _style = document.createElement('style');",
  "  _style.textContent = '@keyframes __kvSpin{to{transform:rotate(360deg)}}';",
  "  document.head.appendChild(_style);",
  "  document.body.appendChild(_overlay);",
  "",
  "  // Tiến trình giả (0→95%) trong suốt thời gian loading",
  "  const _startTime = Date.now();",
  "  const _totalMs = ${loadMs};",
  "  const _ticker = setInterval(()=>{",
  "    const pct = Math.min(95, Math.round((Date.now() - _startTime) / _totalMs * 95));",
  "    const bar = document.getElementById('__kv_bar');",
  "    const pctEl = document.getElementById('__kv_pct');",
  "    if(bar) bar.style.width = pct + '%';",
  "    if(pctEl) pctEl.textContent = pct + '%';",
  "  }, 80);`;",
  "",
  "  /* ---- Phần fetch code từ server + thực thi ---- */",
  "  const fetchAndRun = `",
  "  // ---- Chờ đủ ${loadMs}ms rồi fetch code từ server ----",
  "  setTimeout(async ()=>{",
  "    try{",
  "      const _res = await fetch('${apiUrl}', {",
  "        cache: 'no-store',",
  "        headers: { 'Accept': 'application/json' }",
  "      });",
  "      if(!_res.ok) throw new Error('HTTP ' + _res.status);",
  "      const _data = await _res.json();",
  "      const _code = _data && _data.code ? _data.code : null;",
  "      if(!_code) throw new Error('Không nhận được code từ server');",
  "",
  "      // Tiến trình → 100%",
  "      clearInterval(_ticker);",
  "      const _bar = document.getElementById('__kv_bar');",
  "      const _pctEl = document.getElementById('__kv_pct');",
  "      if(_bar) _bar.style.width = '100%';",
  "      if(_pctEl) _pctEl.textContent = '100%';",
  "",
  "      // Xoá overlay sau 300ms rồi thực thi code",
  "      setTimeout(()=>{",
  "        const _ov = document.getElementById('__kv_loader__');",
  "        if(_ov) _ov.remove();",
  "        // Thực thi code gốc từ server",
  "        try{",
  "          const _fn = new Function(_code);",
  "          _fn();",
  "        }catch(_e){",
  "          console.error('[KeyVault Loader] Lỗi khi thực thi code:', _e);",
  "        }",
  "      }, 300);",
  "    }catch(_e){",
  "      clearInterval(_ticker);",
  "      const _ov = document.getElementById('__kv_loader__');",
  "      if(_ov){",
  "        _ov.innerHTML = \\`<div style='text-align:center'>",
  "          <div style='font-size:32px;margin-bottom:12px'>⚠</div>",
  "          <div style='font-size:14px;font-weight:600;margin-bottom:6px'>Không tải được dữ liệu server</div>",
  "          <div style='font-size:11px;color:#888'></div>",
  "        </div>\\`;",
  "        setTimeout(()=>{ const e=document.getElementById('__kv_loader__'); if(e)e.remove(); }, 3000);",
  "      }",
  "    }",
  "  }, ${loadMs});`;",
  "",
  "  const innerCode = `${antiConsoleBlock}${antiDebugBlock}${antiEditBlock}${loaderUI}${fetchAndRun}`;",
  "",
  ...
  "// @version      1.0",
  "// @description  Tự động tải code \"${snippetName}\" từ KeyVault Server và thực thi",
  "// @author       KeyVault",
  "// @match        ${matchUrl || '*://*/*'}",
  "// @grant        none",
  "// @run-at       document-end",
